...
export default function EditCharacterPage() {
  ...
  useEffect(() => {
    if (dictionaryId && characterId) {
      fetchData();
    } else {
      setLoading(false);
    }
  }, [dictionaryId, characterId]);
  ...
  const fetchData = async () => {
    try {
      ...
      const currentCharacter = charactersData.find(c => c.id === characterId);
      ...
      if (currentCharacter) {
        setFormData({
          ...
          personalityTraits: currentCharacter.personalityTraits?.join(', ') || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!dictionaryId || !characterId) return;
    ...
    try {
      ...
      console.log('Would update character with:', characterData);
  ...
  if (!dictionaryId || !dictionary || !character) {
    return (
      <ProtectedRoute>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Update character details in "{dictionary.title}"
          </p>
        </div>
